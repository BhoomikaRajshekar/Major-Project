...
def send_email_with_attachment(recipient, subject, body, attachment_path):
    ...
    sender_email = "fizanaaz404@gmail.com"  # Replace with your email
    sender_password = "sxcr mzai jldc stgc"        # Replace with your app password
...
def extract_table_data(extracted_text, extracted_sections):
    ...
    pattern = r'((?:\d{2}[A-Z0-9]+)|(?:\d{4}[A-Z0-9]+))\s+([\w\s#]+)\s+(\d+)\s+(\d*)\s+(\d+)\s+([A-ZW])\s+(\d{4}-\d{2}-\d{2})'
...
if __name__ == "__main__":
    ...
    # Fetching arguments passed from PHP script
    ...
    folder_path = f'C:\\wamp64\\www\\project\\templates\\uploads\\{department}\\{scheme}\\{year}'
    ...
    try:
        ...
        save_dir = os.path.join(base_dir, department, scheme, year)
